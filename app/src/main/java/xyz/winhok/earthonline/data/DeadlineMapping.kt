...
fun BackupSnapshot.deadlineState() = DeadlineState(world, DeadlineBook(
    contracts.map { TimeContract(it.id,it.questId,it.occurrence,it.zoneId,it.dueDay,it.originalRewardXp,it.currentRewardXp,it.status,it.signedAt,it.closedAt,it.signedKind,it.signedSkill,it.extensionCount,it.fulfilledAt,it.titleSnapshot) },
...
fun BackupSnapshot.withDeadline(state: DeadlineState): BackupSnapshot = copy(
    world=state.world,
    contracts=state.book.contracts.map { ContractEntity(it.id,it.questId,it.occurrence,it.zoneId,it.dueDay,it.originalRewardXp,it.currentRewardXp,it.status,it.signedAt,it.closedAt,it.signedKind,it.signedSkill,it.extensionCount,it.fulfilledAt,if(it.status=="ACTIVE")it.questId else null,it.titleSnapshot) },
